...
export const Modal = ({className, title, show, name, onHide, children}) => {
	return (
		<div className={`${className ?? ""} ${styles.modal} ${show ? styles.active : ""}`}>
			<div className={styles["modal-content"]}>
				<span className={styles["modal__close"]} onClick={() => onHide.fn({...onHide.state, [name]: !show})}>
				...
				{
					title &&
					<div className={styles["modal-content__head"]}>
						<p className={styles["modal-head__title"]}>{title}</p>
					</div>
				}
			...
			<div className={styles["modal__backdrop"]} onClick={() => onHide.fn({...onHide.state, [name]: !show})}></div>
		</div>
	)
}